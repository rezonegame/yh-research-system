...
| `skill-factory` | Create new general skills and publish-ready perspective skills; retire/archive perspectives | 造skill, 创建技能, 沉淀流程, 蒸馏人物视角, 退休视角 |
| `darwin-skill` | Evaluate, score, optimize, and evolve skills with test prompts and rollback | 达尔文, skill评分, skill质量检查, 自动优化 |
| `perspective-library` | Route requests to active person/thinking-style perspective skills | 用某某视角, 某人会怎么看, perspective, 思维方式 |
...
Person perspectives are now created as publish-ready by default: six research lanes, triple-validated mental models, bibliography, replay/unknown/edge tests, independent fresh-context validation, and Darwin score >= 90. Retirement means archive-first removal from active routing, not hard deletion.
...
| **Socrates** (苏格拉底) | Concept clarification, assumption testing, contradiction exposure, reflective dialogue |
| **Sukhomlinsky** (苏霍姆林斯基) | Humanistic pedagogy, whole-child development, teacher craft, moral education |
| **Piaget** (皮亚杰) | Cognitive development, constructivist learning diagnosis, conceptual change, child reasoning |
...
| And more... | See `perspective-library/INDEX.md` for the active router |
...
| `_archive/` | Deprecated skills, retired perspectives, examples, and legacy material |
...
| `skill-factory` | 创建新通用技能和 publish-ready 人物视角技能；退休/归档视角 | 造skill、创建技能、沉淀流程、蒸馏人物视角、退休视角 |
| `darwin-skill` | 使用测试提示和回滚机制评估、评分、优化和进化技能 | 达尔文、skill评分、skill质量检查、自动优化 |
| `perspective-library` | 将请求路由到 active 人物/思维风格视角技能 | 用某某视角、某人会怎么看、perspective、思维方式 |
| `lecture-slides` | Beamer、LaTeX、Quarto RevealJS、TikZ 提取、参考文献验证、幻灯片 QA、部署 | 创建、编译、转换、审查或部署教学幻灯片 |

### 视角库

人物视角现在默认按 publish-ready 标准创建：六路研究、三重验证心智模型、书目、公开回放/未知问题/边界测试、独立 fresh-context 验证，以及 Darwin 分数 >= 90。退休机制默认为归档优先：从 active 路由移除，但文件保留在 `_archive/deprecated/perspectives/`。
...
| **苏格拉底** | 概念澄清、前提检验、矛盾暴露、反思式对话 |
| **苏霍姆林斯基** | 人道主义教育、完整儿童发展、教师技艺、道德教育 |
| **皮亚杰** | 认知发展、建构主义学习诊断、概念转变、儿童推理 |
...
| 更多... | 见 `perspective-library/INDEX.md` 中的 active router |
